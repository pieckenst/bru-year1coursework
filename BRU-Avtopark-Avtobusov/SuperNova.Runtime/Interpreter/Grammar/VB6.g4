...
doLoopStmt
   : DO NEWLINE + (block NEWLINE +)? LOOP # doBlockLoop
   | DO WS (WHILE | UNTIL) WS valueStmt NEWLINE + (block NEWLINE +)? LOOP # doWhileBlockLoop
   | DO NEWLINE + (block NEWLINE +) LOOP WS (WHILE | UNTIL) WS valueStmt # doBlockWhileLoop
   ;
